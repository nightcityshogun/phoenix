<#
.SYNOPSIS
    Manages Azure Subscription role assignments, including creation, backup, and cleanup.

.DESCRIPTION
    This script connects to Azure, requests operator consent, and provides options to:
    - Create role assignments for Azure Subscriptions.
    - Backup existing role assignments to a CSV file.
    - Clean up role assignments from Azure Subscriptions.
...
.PARAMETER maxSubscriptionsToUse
...
.NOTES
    ...
    Name: Manage_AzureSubscriptionRoleAssignments
    Version: 3.0
    Date: 2023-12-15
#>
...
# Search Pattern
$backupFilePattern = "Manage_AzureSubscriptionRoleAssignments_Backup_*.csv"

# Export File Path
$backupFileName = "Manage_AzureSubscriptionRoleAssignments_Backup" + (Get-Date -Format "yyyyMMdd_HHmmss") + ".csv"
...
Write-Log -Message "[SCRIPT] Start of Manage_AzureSubscriptionRoleAssignments Script." -Level "SOS"
...
function Create-NewRoleAssignments {
    $enabledSubscriptions = Get-AzSubscription | Where-Object { $_.State -eq 'Enabled' } | Select-Object -First $maxSubscriptionsToUse

    $consent = Read-Host -Prompt "Do you wish to create Role Assignments for Subscriptions? (Yes/No)"
    if ($consent -notmatch '^(yes|y)$') {
        Write-Log -Message "Consent has not been provided for Subscription Role Assignment Creation." -Level "ERROR"
        return
    }
    Write-Log -Message "Consent provided for Subscription Role Assignment Creation." -Level "IMPORTANT"

    foreach ($subscription in $enabledSubscriptions) {
        Write-Log -Message "Creating Role Assignments for $($subscription.Name)" -Level "SOS"
        $subscriptionScope = "/subscriptions/$($subscription.Id)"
        $existingAssignments = Get-AzRoleAssignment -Scope $subscriptionScope

        $randomGroups = Get-AzADGroup -Filter "securityEnabled eq true" | Sort-Object DisplayName | Get-Random -Count $maxGroups
        $randomUsers = Get-AzADUser | Where-Object { $_.Id -ne $azContext.Account.Id } | Sort-Object DisplayName | Get-Random -Count $maxUsers
        ...
        foreach ($roleName in ($Roles.Keys | Sort-Object)) {
            ...
            foreach ($entity in $randomGroups) {
                if (-not ($existingAssignments | Where-Object { $_.RoleDefinitionId -eq $roleId -and $_.ObjectId -eq $entity.Id })) {
                    try {
                        New-AzRoleAssignment -ObjectId $entity.Id -RoleDefinitionId $roleId -Scope $subscriptionScope -ErrorAction SilentlyContinue > $null
                        Write-Log -Message "[GROUP] $($entity.DisplayName) assigned with $roleName at $($subscription.Name)." -Level "INFO"
                    } catch {
                        Write-Log -Message "[GROUP] Failed to assign $roleName to $($entity.DisplayName) at $($subscription.Name): $($_.Exception.Message)" -Level "ERROR"
                    }
                }
            }
            ...
            foreach ($user in $randomUsers) {
                if (-not ($existingAssignments | Where-Object { $_.RoleDefinitionId -eq $roleId -and $_.ObjectId -eq $user.Id })) {
                    try {
                        New-AzRoleAssignment -ObjectId $user.Id -RoleDefinitionId $roleId -Scope $subscriptionScope -ErrorAction SilentlyContinue > $null
                        Write-Log -Message "[USER] $($user.UserPrincipalName) assigned with $roleName at $($subscription.Name)." -Level "INFO"
                    } catch {
                        Write-Log -Message "[USER] Failed to assign $roleName to $($user.UserPrincipalName) at $($subscription.Name): $($_.Exception.Message)" -Level "ERROR"
                    }
                }
            }
            $roleCount++
    ...
    Write-Log -Message "Subscription Role Assignment Complete" -Level "SOS"
    SLEEP 2
    CLS
}
...
function Backup-RoleAssignments {
    $AllAssignments = @()
    $AllSubscriptions = Get-AzSubscription | Select-Object -First $maxSubscriptionsToUse

    foreach ($SingleSubscription in $AllSubscriptions) {
        $subscriptionScope = "/subscriptions/$($SingleSubscription.Id)"
        $assignments = Get-AzRoleAssignment -Scope $subscriptionScope
        $AllAssignments += $assignments
        Write-Log -Message "Collected assignments for Subscription '$($SingleSubscription.Name)'" -Level "INFO"
    }
    ...
    Write-Log -Message "Subscription Role Assignment Backup Complete. File saved to: $backupFilePath" -Level "IMPORTANT"
    SLEEP 2
    CLS
}

# Function to Clean Subscriptions
function Clean-Subscriptions {
    param (
    ...
    # Retrieve all Enabled Subscriptions
    $enabledSubscriptions = Get-AzSubscription | Where-Object { $_.State -eq 'Enabled' }
    if ($enabledSubscriptions.Count -eq 0) {
        Write-Log -Message "No enabled subscriptions found." -Level "ERROR"
        return
    }

    # Prompt for consent to clean Role Assignments
    $consent = Read-Host -Prompt "Do you wish to start cleaning Role Assignments for Subscriptions? (Yes/No)"
    if ($consent -notmatch '^(yes|y)$') {
        Write-Log -Message "Consent not provided for Subscription Role Assignment cleanup." -Level "ERROR"
        return
    }

    Write-Log -Message "Consent provided for Subscription Role Assignment cleanup." -Level "IMPORTANT"
    ...
    foreach ($subscription in $enabledSubscriptions) {
        ...
        Write-Log -Message "Starting cleanup for Subscription: $subscriptionDisplayName. Excluding User: $loggedOnUserUPN." -Level "IMPORTANT"

        # Remove 'User Access Administrator' assignments excluding logged-on user
        $uaAssignments = Get-AzRoleAssignment -Scope $subscriptionScope | Where-Object {
        ...
        foreach ($uaAssignment in $uaAssignments) {
            ...
            try {
                Remove-AzRoleAssignment -ObjectId $uaAssignment.ObjectId -RoleDefinitionName "User Access Administrator" -Scope $subscriptionScope -ErrorAction SilentlyContinue > $null
                
                Write-Log -Message "$tag $displayName removed from Role: User Access Administrator in Subscription: $subscriptionDisplayName." -Level "INFO"
            } catch {
                Write-Log -Message "[ERROR] Failed to remove $tag $displayName from Role: User Access Administrator in Subscription: $subscriptionDisplayName. Error: $($_.Exception.Message)" -Level "ERROR"
            }
        }

        # Remove all other Role Assignments excluding logged-on user
        $roleAssignments = Get-AzRoleAssignment -Scope $subscriptionScope | Where-Object {
        ...
        foreach ($roleAssignment in $roleAssignments) {
            ...
            try {
                Remove-AzRoleAssignment -ObjectId $roleAssignment.ObjectId -RoleDefinitionName $roleDefinitionName -Scope $subscriptionScope -ErrorAction SilentlyContinue > $null

                Write-Log -Message "$tag $displayName removed from Role: $roleDefinitionName in Subscription: $subscriptionDisplayName." -Level "INFO"
            } catch {
                Write-Log -Message "[ERROR] Failed to remove $tag $displayName from Role: $roleDefinitionName in Subscription: $subscriptionDisplayName. Error: $($_.Exception.Message)" -Level "ERROR"
            }
        }
    }
    Write-Log -Message "Subscription Role Assignments cleaned." -Level "SOS"
    SLEEP 2
    CLS
}
...
do {
    ...
    Write-Host "[1] Create Subscription Role Assignments"
    Write-Host "[2] Backup Subscription Role Assignments"
    Write-Host "[3] Clean Subscription Role Assignments"
    Write-Host "[4] Exit"
    Write-Host ""
    Write-Host "----------------------------------------------------------------------------------"
    ...
    switch ($option) {
        ...
        "3" { 
             Clean-Subscriptions -loggedOnUserInfo $loggedOnUserInfo
            }
        "4" {
            ...
            Write-Log -Message "[SCRIPT] End of Manage_AzureSubscriptionRoleAssignments Script." -Level "SOS"
            SLEEP 2
            CLS
            exit
        }
