...
function Set-GMSA {
    ...
    begin {
        ...
        $doExec = [bool]$Execute
        $whatIf = -not $doExec
        # --- helpers (minimal & integrated) ---
        $GetDe = { param([string]$Path) [ADSI]$Path }
        $WI = { param([string]$Message) if ($whatIf) { Write-IdentIRLog -Message "[WHATIF] $Message" -TypeName 'Info' -ForegroundColor Green } }
        function Get-KdsRootKeys {
            param([string]$BindServer, [scriptblock]$GetDe)
            ...
            try {
                ...
                foreach ($r in $ds.FindAll()) {
                    ...
                    $keys.Add([pscustomobject]@{ DistinguishedName=$dn; CN=$cn; WhenCreated=$wc })
                }
            } catch { }
            return ,$keys
        }
        function Remove-KdsRootKeys {
            param([string]$BindServer, [scriptblock]$GetDe, [psobject[]]$OldKeys)
            $count = 0
        ...
        function Restart-KdsService {
            ...
            if ($whatIf) { & $WI ("kdsSvc restarted on {0}" -f $ComputerName); return }
            try {
                $c = @{ ClassName='Win32_Service'; Filter="Name='kdsSvc'"; ComputerName=$ComputerName }
                $svc = Get-CimInstance @c
                if (-not $svc) { Write-IdentIRLog -Message "kdsSvc not found on ${ComputerName}" -TypeName 'Warning' -ForegroundColor Yellow; return }
                $null = $svc | Invoke-CimMethod -MethodName StopService | Out-Null
                $deadline = (Get-Date).AddSeconds(60)
                while ($svc.State -ne 'Stopped' -and (Get-Date) -lt $deadline) { $svc = Get-CimInstance @c; Start-Sleep -Milliseconds 200 }
                if ($svc.State -ne 'Stopped') { Write-IdentIRLog -Message "kdsSvc stop timed out on ${ComputerName}" -TypeName 'Warning' -ForegroundColor Yellow }
                $null = $svc | Invoke-CimMethod -MethodName StartService | Out-Null
                $deadline = (Get-Date).AddSeconds(60)
                while ($svc.State -ne 'Running' -and (Get-Date) -lt $deadline) { $svc = Get-CimInstance @c; Start-Sleep -Milliseconds 200 }
                if ($svc.State -ne 'Running') { Write-IdentIRLog -Message "kdsSvc start timed out on ${ComputerName}" -TypeName 'Warning' -ForegroundColor Yellow }
        ...
        function Force-ConfigReplication {
            param([string]$SourceDC, [string]$CfgNC)
            try {
        ...
        $in = @($IsolatedDCList)
        if (-not $in -or $in.Count -eq 0) { Write-IdentIRLog -Message "IsolatedDCList is empty." -TypeName 'Error' -ForegroundColor Red; return }
        ...
        if (@($validated).Count -eq 0) { Write-IdentIRLog -Message "No valid DC entries." -TypeName 'Warning' -ForegroundColor Yellow; return }
        # Single forest check
        ...
        if ($forestRoots.Count -gt 1) { Write-IdentIRLog -Message "Multiple forests detected in input. This function supports one forest only." -TypeName 'Error' -ForegroundColor Red; return }
        if ($forestRoots.Count -eq 0) { Write-IdentIRLog -Message "No forest root NC detected." -TypeName 'Error' -ForegroundColor Red; return }
        ...
        foreach ($g in $groups) {
            ...
            $meta[$g.Name] = @{ Type=$rep.Type; DefaultNC=$rep.DefaultNamingContext; Domain=$g.Name }
        }
        ...
        $script:DesiredUsableTime = if ([bool]$EffectiveImmediately -or ($EffectiveTimeHoursOffset -eq 0)) { Get-Date } else { (Get-Date).AddHours($EffectiveTimeHoursOffset) }
        ...
        # Choose bind DC
        $scanOrder = @($validated | Where-Object { $_.IsPdcRoleOwner -and $_.Online }) + @($validated | Where-Object { -not $_.IsPdcRoleOwner -and $_.Online })
        $script:kdsBindServer = $null; $script:existingKeyCount = 0; $script:oldKeys = @()
        foreach ($dc in $scanOrder) {
            try {
                $keys = Get-KdsRootKeys -BindServer $dc.FQDN -GetDe $GetDe
                if ($keys.Count -gt 0) { $script:kdsBindServer=$dc.FQDN; $script:existingKeyCount=$keys.Count; $script:oldKeys=$keys; Write-IdentIRLog -Message ("Observed {0} existing KDS root key(s) via {1}; will recreate using new key and delete old at end." -f $keys.Count, $dc.FQDN) -TypeName 'Info' -ForegroundColor Green; break }
            } catch { }
        }
        if (-not $script:kdsBindServer) {
            ...
            if ($forestRootSet.Count -gt 0) {
                $frOrdered = @($forestRootSet | Where-Object { $_.IsPdcRoleOwner }) + @($forestRootSet | Where-Object { -not $_.IsPdcRoleOwner })
        ...
        if ($whatIf) {
            $effLabel = if ($script:DesiredUsableTime -le (Get-Date)) { "EffectiveImmediately" } else { "EffectiveTime=$($script:DesiredUsableTime)" }
            ...
            if ($creationOk) {
                ...
                do {
                    ...
                    $newKey = $afterKeys | Where-Object { $_.CN -notin $script:oldKeys.CN } | Sort-Object WhenCreated -Descending | Select-Object -First 1
                    $script:newKdsCn = $newKey.CN
                } until ($script:newKdsCn -or (Get-Date) -gt $keyTimeout)
                if (-not $script:newKdsCn) { Write-IdentIRLog -Message "CRITICAL: New KDS root key not visible after timeout." -TypeName 'Error' -ForegroundColor Red; return }
                else { Write-IdentIRLog -Message "New KDS root key presence verified: $($script:newKdsCn)" -TypeName 'Info' -ForegroundColor Green }
            }
    ...
    process { }
        ...
        foreach ($domain in $script:orderedDomains) {
            ...
            if ($dcs -and $dcs[0].PSObject.Properties['IsRODC']) { $dcs = @($dcs | Where-Object { -not $_.IsRODC }) }
            if (-not $dcs) { Write-IdentIRLog -Message "No online writable DCs for $domain. Skipping." -TypeName 'Error' -ForegroundColor Red; continue }
            $orderedDcs = @()
            ...
            if ($whatIf) {
                foreach ($c in ($orderedDcs | ForEach-Object { $_.FQDN } | Sort-Object -Unique)) { & $WI ("kdsSvc restarted on {0}" -f $c) }
            } else {
                $targets = ($orderedDcs | ForEach-Object { $_.FQDN } | Sort-Object -Unique)
                $iss = [System.Management.Automation.Runspaces.InitialSessionState]::CreateDefault()
                $pool = [RunspaceFactory]::CreateRunspacePool(1, 16, $iss, $Host); $pool.Open()
                $rsJobs = @()
                foreach ($t in $targets) {
                    ...
                    $null = $ps.AddScript({
                        ...
                        } catch { return @{Computer=$ComputerName; Success=$false; Error=$_.Exception.Message} }
                    }).AddArgument($t)
                ...
                foreach ($j in $rsJobs) {
                    ...
                    foreach ($x in $r) {
                        if ($x.Success) { Write-IdentIRLog -Message "kdsSvc restarted on $($x.Computer)" -TypeName 'Info' -ForegroundColor Green }
                        else { Write-IdentIRLog -Message "kdsSvc restart failed on $($x.Computer): $($x.Error)" -TypeName 'Warning' -ForegroundColor Yellow }
                    }
                }
            ...
            try {
                ...
                foreach ($r in $ds.FindAll()) {
                    ...
                    $gmsaIndex.Add([pscustomobject]@{ DistinguishedName=$dn; ParentContainerDN=$parent; SamAccountName=$sam })
                }
            } catch {
            ...
            # Disable & tag → clear SPNs/DNS → delete → confirm 
            $oldToNewMap = @{}
            $firstNewDn = $null
            foreach ($g in $gmsaIndex) {
                ...
                $exp = @{
                    ...
                    ServicePrincipalName=@(); ManagedPasswordInterval=30
                    AllowedToDelegateTo=@(); UserAccountControl=$null
                    DnsHostName=$null; GroupMSAMembership=$null
                    AllowedToActOnBehalfOfOtherIdentity=$null; SupportedEncryptionTypes=$null
                }
                try {
                ...
                # 1) Disable & tag OLD
                if ($whatIf) { & $WI ("Disabled & Tagged gMSA: {0}" -f $oldDn) }
                else {
                    try {
                        ...
                        $de.SetInfo()
                        Write-IdentIRLog -Message "Disabled & tagged gMSA: ${oldDn}" -TypeName 'Info' -ForegroundColor Green
                    } catch {
                ...
                if ($whatIf) { & $WI ("Clear SPNs and dnsHostName on {0}" -f $oldDn) }
                else {
                    try {
                        $de = & $script:GetDe ("LDAP://${bindServer}/${oldDn}")
                ...
                # 3) Delete OLD
                if ($whatIf) { & $WI ("Deleted {0}" -f $oldDn) }
                else {
                    try {
                ...
                # 4) Confirm CN gone (run in both modes; prints failure line if still present)
                $ou = & $script:GetDe ("LDAP://${bindServer}/${parent}")
                $exists = $false
                if (-not $whatIf) {
                    $exists = $true; $retryCount = 0; $maxRetries = 5
                ...
                if ($exists) {
                    Write-IdentIRLog -Message "Failed to confirm deletion of $cn after retries. Skipping creation." -TypeName 'Warning' -ForegroundColor Yellow
                    continue
                }
                # Normalize sAM (end with $; <=15 chars)
                if ($sam -and -not $sam.EndsWith('$')) { $sam = "${sam}$" }
                if ($sam -and $sam.Length -gt 15) {
                    $oldSam = $sam; $sam = $sam.Substring(0,15)
                ...
                # 5) Recreate NEW (same CN) — WhatIf prints only a simulation line
                $newDn = $null
                if ($whatIf) {
                    ...
                    try {
                        ...
                        $new.SetInfo()
                        try { foreach ($spn in @($exp.ServicePrincipalName)) { [void]$new.Properties["servicePrincipalName"].Add($spn) } } catch { Write-IdentIRLog -Message "Adding SPNs failed on CN=$cn,${parent}: $($_.Exception.Message)" -TypeName 'Warning' -ForegroundColor Yellow }
                        try { foreach ($svc in @($exp.AllowedToDelegateTo)) { [void]$new.Properties["msDS-AllowedToDelegateTo"].Add($svc) } } catch { Write-IdentIRLog -Message "Adding AllowedToDelegateTo failed on CN=$cn,${parent}: $($_.Exception.Message)" -TypeName 'Warning' -ForegroundColor Yellow }
                        if ($exp.ServicePrincipalName.Count -gt 0 -or $exp.AllowedToDelegateTo.Count -gt 0) { $new.SetInfo() }
                        $newDn = "CN=$cn,$parent"
                    ...
                    try {
                        ...
                        foreach ($r in $ds.FindAll()) {
                            ...
                            for ($i = 0; $i -lt $currentAccounts.Count; $i++) {
                                if ($oldToNewMap.ContainsKey($currentAccounts[$i])) { $currentAccounts[$i] = $oldToNewMap[$currentAccounts[$i]]; $updated = $true }
                            }
                            if ($updated) {
                                try {
                                    $cde = & $script:GetDe ("LDAP://${bindServer}/${compDn}")
                                    $cde.Properties['msDS-HostServiceAccount'].Clear()
                                    foreach ($acc in $currentAccounts) { [void]$cde.Properties['msDS-HostServiceAccount'].Add($acc) }
                                    $cde.SetInfo()
        ...
        if ($script:existingKeyCount -gt 0) {
            try {
                ...
                if ($deleted -gt 0) { Write-IdentIRLog -Message ("Deleted old KDS root key(s) (forest-wide).") -TypeName 'Info' -ForegroundColor Yellow }
                Force-ConfigReplication -SourceDC $script:kdsBindServer -CfgNC $script:cfgNC
            } catch {
        ...
        if ($whatIf) {
            ...
            $poolF = [RunspaceFactory]::CreateRunspacePool(1, 16, $issF, $Host); $poolF.Open()
            $jobsF = @()
            foreach ($c in $allDcs) {
                $ps = [PowerShell]::Create(); $ps.RunspacePool = $poolF
                $null = $ps.AddScript({
                    param($ComputerName)
                    try {
                        $c = @{ ClassName='Win32_Service'; Filter="Name='kdsSvc'"; ComputerName=$ComputerName }
