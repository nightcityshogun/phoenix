...
(effective now or offset). It restarts kdsSvc on the forest-root PDC only, disables and removes existing gMSAs,
...
Behavior: Creates new KDS root key on forest-root PDC, forces replication from that PDC via repadmin started with CIM,
restarts kdsSvc on forest-root PDC, recreates gMSAs, optionally rebinds hosts, removes old keys.
...
function Set-GMSA {
    ...
    begin {
        ...
        function Restart-KdsService {
            ...
            # NOTE: this is only called against the forest-root PDC.
            if ($whatIf) {
        ...
        function Invoke-AddKdsRootKeyRemote {
            ...
            try {
                ...
                # Build a PowerShell one-liner that parses the ISO time and runs Add-KdsRootKey
                $ps = "[datetime]`$t = [datetime]::ParseExact('$iso','o',[System.Globalization.CultureInfo]::InvariantCulture); Add-KdsRootKey -EffectiveTime `$t"
        ...
        if ($forestRoots.Count -eq 0) {
            Write-IdentIRLog -Message "No forest root NC detected from supplied DCs. Ensure at least one online DC is reachable." -TypeName 'Error' -ForegroundColor Red
            return
        }
        ...
        # NOTE: This must be present in the supplied IsolatedDCList; if not, we exit cleanly.
        ...
        if (-not $forestRootPdc) {
            $msg = "Forest root PDC for '$forestRootDomain' is not present in the supplied IsolatedDCList or is offline. " +
                   "Start the forest-root domain controller or perform PDC FSMO seizure, then rerun Set-GMSA. No changes made."
            Write-IdentIRLog -Message $msg -TypeName 'Error' -ForegroundColor Red
            return
        }
        ...
        if ($whatIf) {
            ...
            if (-not $creationOk) {
                Write-IdentIRLog -Message "KDS root key creation FAILED on $($script:kdsBindServer). Aborting, no gMSA changes made." -TypeName 'Error' -ForegroundColor Red
                return
            }
            ...
            if (-not $script:newKdsCn) {
                Write-IdentIRLog -Message "New KDS root key not visible on forest-root PDC after timeout. Aborting." -TypeName 'Error' -ForegroundColor Red
                return
            }

            Write-IdentIRLog -Message "New KDS root key visible on forest-root PDC: $($script:newKdsCn)" -TypeName 'Info' -ForegroundColor Green

            # Restart kdsSvc ONLY on forest-root PDC (via CIM)
    ...
    process { }
        ...
        foreach ($domain in $script:orderedDomains) {
            ...
            # Enumerate gMSAs for this domain
            ...
            # If no gMSAs in this domain, skip processing cleanly
            if ($gmsaIndex.Count -eq 0) {
                Write-IdentIRLog -Message "No gMSA objects present in $domain. Skipping domain." -TypeName 'Info' -ForegroundColor White
                continue
            }
            ...
            foreach ($g in $gmsaIndex) {
                ...
                # backup attributes (best-effort, ADSI only)
                $exp = @{
                    DisplayName                         = $null
                    Description                         = $null
        ...
        # Old KDS key cleanup on forest-root PDC (if we had keys before)
        if ($script:existingKeyCount -gt 0 -and $script:newKdsCn) {
            try {
                ...
                # Replicate config from forest-root PDC after key deletion
                Force-ConfigReplication -SourceDC $script:kdsBindServer -CfgNC $script:cfgNC

                # Restart kdsSvc ONLY on forest-root PDC after key deletion
                Restart-KdsService -ComputerName $script:kdsBindServer
            } catch {
